...
/// Represents a downloadable or local language model.
...
class LlamaState: ObservableObject {
    ...
    /// Path to the bundled default Gemma 3n model
    ...
    init() {
        ...
        /// Attempt to load the Gemma 3n model from bundle
        if let url = defaultModelUrl {
            do {
                try loadModel(modelUrl: url)
                print("✅ Gemma 3n model loaded successfully")
            } catch {
                print("❌ Failed to load Gemma 3n model: \(error)")
            }
        }

    }

    /// Scans the app’s documents directory for any locally downloaded models
    ...
    /// Adds predefined models to the undownloaded list if not already found locally
    ...
    /// Default library of known models
    ...
    /// Attempts to initialize a model at the provided URL path
    ...
    /// Updates model status to reflect successful download or usage
    private func updateDownloadedModels(modelName: String, status: String) {
        undownloadedModels.removeAll { $0.name == modelName }
    }

    /// Sends a text prompt to the LLM and prints token generation stats
    ...
    /// Benchmarks model generation speed and readiness
    func bench() async {
        guard let llamaContext else {
            return
        }
    ...
    /// Clears current context and resets message log
    func clear() async {
        guard let llamaContext else {
            return
        }
